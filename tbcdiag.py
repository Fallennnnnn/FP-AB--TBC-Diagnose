...
st.header("Tuberculosis Diagnose with Machine Learning")
# label for machine and set file uploader in streamlit
lab = {'Tuberculosis': 0, 'Normal': 1}
diagpic = st.file_uploader("Please Upload the X-Ray TBC Diagnosis File", type=["jpg", "png", "jpeg"])
...
with st.container():
    def diagnosis(file):
        ...
        with col2:
            st.markdown("<h1 style='text-align: center; color: black;'>TBC Diagnosis Picture</h1>", unsafe_allow_html=True)
    ...
    ##YOUR CODE GOES HERE##
            plt.imshow(image)
            st.image(image,caption= "X-Ray TBC Diagnosis Picture" , channels="RGB", width=250)
...
if diagpic is None:
   ...
   diagres = st.text(diagnosis(diagpic))
